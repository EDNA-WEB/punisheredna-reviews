...
export default async function AllNewReviewsPage({ searchParams }: { searchParams: { page?: string } }) {
  ...
  const [reviews, total] = await Promise.all([
    ...
    prisma.review.count({ where: { seasonId: null, episodeId: null } })
  ]);
        ...
        <>
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-center gap-3">
              {page > 1 && (
                <Link href={`/recenzie/nove?page=${page - 1}`} className="text-sm font-semibold px-4 py-2 rounded-full border border-line hover:border-accent hover:text-accent">
                  ← Novšie
                </Link>
              )}
