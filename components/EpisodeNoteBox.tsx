...
export default function EpisodeNoteBox({ episodeId, initialBody }: { episodeId: string; initialBody: string }) {
  ...
  async function save() {
    setLoading(true);
    try {
      const res = await fetch('/api/episode-notes', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ episodeId, body: text })
      });
