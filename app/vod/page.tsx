...
import { getCachedVodPremieres } from '@/lib/cachedMovieData';
...
export default async function VodPage({ searchParams }: { searchParams: { month?: string; year?: string } }) {
  ...
  // Pozor: dáta z cachovanej funkcie prešli cez JSON serializáciu, takže
  // "releaseDate" je tu REŤAZEC, nie skutočný Date objekt — všade nižšie sa
  // preto vždy najprv obalí do "new Date(...)", než sa naň zavolá dátumová
  // metóda (presne toto bolo príčinou pádu stránky pri predošlom pokuse).
  const { movies, people } = await getCachedVodPremieres(rangeStart.toISOString(), rangeEnd.toISOString());
